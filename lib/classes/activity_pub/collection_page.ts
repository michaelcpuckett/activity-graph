...
export class APCollectionPage extends APCoreObject implements AP.CollectionPage {
  ...
  constructor(collectionPage: AP.AnyCollectionPage) {
    ...
    if (Object.values(AP.CollectionPageTypes).includes(collectionPage.type)) {
      ...
      throw new Error('`type` must be defined and be one of the CollectionPage Types.');
    }
  }
};

export class APOrderedCollectionPage extends APCollectionPage implements AP.OrderedCollectionPage {
  ...
  constructor(collectionPage: AP.OrderedCollectionPage) {
    ...
    if (collectionPage.type !== AP.CollectionPageTypes.ORDERED_COLLECTION_PAGE) {
