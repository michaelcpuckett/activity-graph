...
export default userHandler(
  serviceAccount,
  async (actor: AP.Actor, graph: Graph) => {
    if (!actor.id) {
      return;
    }

    const pokemonCollectionId = `${PROTOCOL}//${LOCAL_HOSTNAME}${
      PORT ? `:${PORT}` : ''
    }/actor/${actor.preferredUsername}/pokemon`;
    ...
    const visitedCollectionId = `${PROTOCOL}//${LOCAL_HOSTNAME}${
      PORT ? `:${PORT}` : ''
    }/actor/${actor.preferredUsername}/visited`;
    ...
    if (actor.streams) {
      ...
      actor.streams = [
        new URL(pokemonCollectionId),
        new URL(visitedCollectionId),
      ];
    }
